import{t as e}from"./shaderStore-D-XQlhUT.js";import"./oitFragment-CEnrraeO.js";import"./mainUVVaryingDeclaration-BBamQlxR.js";var t=`pbrFragmentExtraDeclaration`,n=`varying vec3 vPositionW;
#if DEBUGMODE>0
...
#endif
`;e.IncludesShadersStore[t]||(e.IncludesShadersStore[t]=n);var r={name:t,shader:n},i=`subSurfaceScatteringFunctions`,a=`bool testLightingForSSS(float diffusionProfile)
{return diffusionProfile<1.;}`;e.IncludesShadersStore[i]||(e.IncludesShadersStore[i]=a);var o={name:i,shader:a},s=`importanceSampling`,c=`vec3 hemisphereCosSample(vec2 u) {float phi=2.*PI*u.x;float cosTheta2=1.-u.y;float cosTheta=sqrt(cosTheta2);float sinTheta=sqrt(1.-cosTheta2);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}
...
float phi=2.*PI*u.x;float sinTheta=pow(u.y,a/(2.*a+1.));float cosTheta=sqrt(1.-sinTheta*sinTheta);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}`;e.IncludesShadersStore[s]||(e.IncludesShadersStore[s]=c);var l={name:s,shader:c},u=`pbrHelperFunctions`,d=`#define MINIMUMVARIANCE 0.0005
...
#endif
`;e.IncludesShadersStore[u]||(e.IncludesShadersStore[u]=d);var f={name:u,shader:d},p=`pbrDirectLightingSetupFunctions`,m=`struct preLightingInfo
...
result.areaLightDiffuse=data.Diffuse;result.LdotV=0.;result.roughness=0.;result.diffuseRoughness=0.;result.surfaceAlbedo=vec3(0.);return result;}
#endif
`;e.IncludesShadersStore[p]||(e.IncludesShadersStore[p]=m);var h={name:p,shader:m},g=`pbrDirectLightingFalloffFunctions`,_=`float computeDistanceLightFalloff_Standard(vec3 lightOffset,float range)
...
#endif
}`;e.IncludesShadersStore[g]||(e.IncludesShadersStore[g]=_);var v={name:g,shader:_},y=`hdrFilteringFunctions`,b=`#if NUM_SAMPLES
#if NUM_SAMPLES>0
...
#endif
#endif
`;e.IncludesShadersStore[y]||(e.IncludesShadersStore[y]=b);var x={name:y,shader:b},S=`pbrDirectLightingFunctions`,C=`#define CLEARCOATREFLECTANCE90 1.0
struct lightingInfo
{vec3 diffuse;
...
#endif
`;e.IncludesShadersStore[S]||(e.IncludesShadersStore[S]=C);var w={name:S,shader:C},T=`pbrBlockNormalGeometric`,E=`vec3 viewDirectionW=normalize(vEyePosition.xyz-vPositionW);
#ifdef NORMAL
...
#endif
`;e.IncludesShadersStore[T]||(e.IncludesShadersStore[T]=E);var D={name:T,shader:E},O=`pbrBlockImageProcessing`,k=`#if defined(IMAGEPROCESSINGPOSTPROCESS) || defined(SS_SCATTERING)
...
#endif
`;e.IncludesShadersStore[O]||(e.IncludesShadersStore[O]=k);var A={name:O,shader:k},j=`pbrDebug`,M=`#if DEBUGMODE>0
...
#endif
`;e.IncludesShadersStore[j]||(e.IncludesShadersStore[j]=M);var N={name:j,shader:M};export{x as a,f as c,r as d,w as i,l,A as n,v as o,D as r,h as s,N as t,o as u};